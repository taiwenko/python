...
for (dev, serial) in serial_glob('loon', 'gxprbridge', 3):
  try:
    units.append(multitest.UnitUnderTest(dev, logdir=args.logfile, serial=serial, aero_path=aero_path))
  except Exception, e:
  ...
  while channel != int(args.max_uut):
    ...
    for unit in units:
      ...
      if match[0] == '0':
         ...
         print match
         gxpr_sn = match[1][1]
         print "RF switch channel " + str(channel) + " is connected to GXPR S/N " + gxpr_sn + " and Bridge Unit " + str(count)
