...
class UnitUnderTest(object):
  ...
  def __init__(self, dev, logdir='.', serial=None, aero_path='.', gxprserial=None):
    ...
    self._serial = serial
    self._gxprserial = gxprserial
    # Open logfile and check if file exists before write
    logfile = '%s-temptest-%s-%s.csv' % (logdir, self._serial, self._gxprserial)
  ...
  def _init(self):
    ...
    self._tom.force_human_mode()
    self._sb = shell.Scoreboard(self._tom, 'gxpr_bridge')

  
  def _reinit(self):
    print 'ALERT: Lost communication with S/N %s %s, resetting' % (self._serial, self._gxprserial)
    sleep(5)
    self._tom.force_human_mode()
    self._tom.reset()

  def shell_log(self, logstr, *args, **kwargs):
    print logstr, args, kwargs
    pass
