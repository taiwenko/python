...
class CommandFailure(Exception):
  ...
  def __str__(self):
    return 'cmd (%s) failed with response (%s)' % (self.cmd, self.response)
...
class LazyScoreboardEntry(lazy.LazyNamedValue):
  ...
  def __init__(self, sb, name, eval_func=None):
    ...
    if not eval_func:
      self.age = LazyScoreboardEntry(sb, name + '.age', self._eval_age)
      self.raw_age_value = LazyScoreboardEntry(sb, name + '.raw_age_value',
...
class Scoreboard(object):
    ...
    try:
      while time.time() - start_time < timeout:
        ...
        if pretty_predicate != last_predicate:
          self._log('    %s' % pretty_predicate, deadline=self._deadline)
  ...
  def get_version_hashes(self):
    versions_by_name = {}
    version_hash_prefix = 'version_hash_'
    versions = self.query(version_hash_prefix)
    for version_hash_name, version_hash in versions.iteritems():
      age, value = version_hash
      if value and (version_hash_name == 'version_hash_pfc' or
                    age < WOERMS_UPDATE_PERIOD):
        name = version_hash_name[len(version_hash_prefix):]
        versions_by_name[name] = version_hash[1]
...
class Shell(pexpect.spawn):
  ...
  def reset(self):
    """Move pexpect's read cursor up to the most recent prompt.
        ...
        """
    while (True):
      index = self.expect([pexpect.TIMEOUT, SHELL_PROMPT], timeout=0.1)
      if index == 0:
        break

  def vars(self, node=None, pattern=None):
    """Returns the result of the 'var' command.

        Returns a mapping of all built-in variables and their current values.
        """
    pattern = pattern if pattern is not None else ''
    node = node if node is not None else 'pfc'
    var_map = utils.DictAttrReadAdapter()
    var_pattern = '([\.\w]+)\t(.+?)\r'
    self.sendline('var @%s %s' % (node, pattern))
  ...
  def send_command(self, command, timeout=5):
    ...
    if index == 1:
      ...
      raise CommandFailure(command, response)
  ...
  def sendline(self, s=''):
    ...
    while retries > 0:
      ...
      if self.expect_exact([pexpect.TIMEOUT, s], timeout=0.25) == 1:
        self.send('\n')
        return
      self.log('Shell corruption! sent %r, received %r' % (s, self.before))
      self.send('\b' * len(s))
  ...
  def force_human_mode(self):
    self.sendline()
    self.expect(SHELL_PROMPT, timeout=1)
